...
public abstract class GSYVideoPlayer extends GSYBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    private boolean mIsForceChangeState;//是否强制转换播放状态
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            Debuger.printfLog("mCurrentState: " + mCurrentState);
    ...
    public void onVideoResume() {
        ...
        if (mCurrentState == CURRENT_STATE_PAUSE) {
            if (mCurrentPosition > 0 && GSYVideoManager.instance().getMediaPlayer() != null) {
                setStateAndUi(CURRENT_STATE_PLAYING);
                GSYVideoManager.instance().getMediaPlayer().seekTo((int)mCurrentPosition);
                GSYVideoManager.instance().getMediaPlayer().start();
            }
        }
    }
...
//                        mSeekTimePosition = (int) (mDownPosition + deltaX * totalTimeDuration / mScreenWidth);
                        mSeekTimePosition = (int) (mDownPosition + deltaX * 1000 / 20);
    ...
    public void onPrepared() {
        ...
        if (GSYVideoManager.instance().getMediaPlayer() != null && mSeekOnStart > 0) {
            GSYVideoManager.instance().getMediaPlayer().seekTo((int)mSeekOnStart);
            mSeekOnStart = 0;
        }
    ...
    public void onInfo(int what, int extra) {
        if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
            ...
            if (BACKUP_PLAYING_BUFFERING_STATE != -1) {
                ...
                if (mIsForceChangeState) {
                    // 因部分机型在播放在线视频时会出现拖动进度条之后等待框一直不消失的问题，所以必须在此增加强制转换状态来解决此问题
                    mIsForceChangeState = false;
                    setStateAndUi(CURRENT_STATE_PLAYING);
                }
